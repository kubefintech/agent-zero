...
class Memory:
        ...
        if Memory.index.get(ctxid) is None:
            ...
            if agent.config.knowledge_subdirs:
                await wrap.preload_knowledge(
                    log_item, agent.config.knowledge_subdirs, memory_subdir, ctxid
                )
            return wrap
        else:
            return Memory(
    ...
    async def preload_knowledge(
        self, log_item: LogItem | None, kn_dirs: list[str], memory_subdir: str, ctxid: str
    ):
        if log_item:
    ...
    def _save_db(self):
        self.db.save_local(folder_path=self._abs_db_dir(self.memory_subdir, self.ctxid))
